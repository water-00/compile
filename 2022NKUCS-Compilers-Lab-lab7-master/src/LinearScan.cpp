    ...
    for(auto &interval:intervals)
    {
        ...
        for (auto use: interval->uses) {
            ...
            if (interval->disp > 255 || interval->disp < -255) {
                ...
                MachineInstruction *inst1 = new LoadMInstruction(use->getParent()->getParent(), vreg, offset);
                use->getParent()->insertBefore(inst1);

                // ldr r_use, [fp, vreg]
                MachineInstruction *inst2 = new LoadMInstruction(use->getParent()->getParent(), use, fp, new MachineOperand(*vreg));
                use->getParent()->insertBefore(inst2);
            } else {
                // 如果没超过，在使用前直接从disp处加载即可: ldr r_use, [fp, offset]
                MachineInstruction *inst = new LoadMInstruction(use->getParent()->getParent(), use, fp, offset);
        ...
        for (auto def: interval->defs) {
            if (interval->disp > 255 || interval->disp < -255) {
                ...
                MachineInstruction *inst1 = new LoadMInstruction(def->getParent()->getParent(), vreg, offset);
                def->getParent()->insertBack(inst1);

                // str r_def ... TODO
                MachineInstruction *inst2 = new StoreMInstruction(def->getParent()->getParent(), def, fp, new MachineOperand(*vreg));
                inst1->insertBack(inst2);

            } else {
                MachineInstruction *inst = new StoreMInstruction(def->getParent()->getParent(), def, fp, offset);
