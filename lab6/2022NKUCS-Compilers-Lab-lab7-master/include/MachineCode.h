...
public:
    MachineBlock* parent;
    enum condType { EQ, NE, LT, LE, GT, GE, NONE }; // 改一下这里
    ...
    void insertBack(MachineInstruction*);
    MachineBlock* getParent() const { return parent; };//2012679 增
    bool isBX() const { return type == BRANCH && op == 2; };//2012679 增 
    //注意不能少op == 2这个条件，否则在遇到b .L6指令也会被认为isBX，AC数+3
    bool isStore() const { return type == STORE; };//2012679 增
    
    bool isAdd() const { return type == BINARY && op == 0; };//2012679 增
};
...
public:
    ...
    MachineBlock* getMachineBlock() { return parent->getParent();};
    void PrintReg();
    void output();
    void setVal(int val) { this->val = val; };//2012679 增
};
...
public:
    ...
    //StackMInstruction(MachineBlock* p, int op, 
     //           MachineOperand* src,
     //           int cond = MachineInstruction::NONE);
     // 2012679 修改参数，增加了std::vector<MachineOperand*> srcs和 MachineOperand* src1 = nullptr
     StackMInstruction(MachineBlock* p, int op, 
     std::vector<MachineOperand*> srcs, MachineOperand* src, MachineOperand* src1 = nullptr, 
     int cond = MachineInstruction::NONE);
     void output();
};
...
private:
    ...
    int cmpno;  //2012679 增 记录前一条 CmpInstruction 的条件码
...
public:
    ...
    std::vector<MachineInstruction*>::reverse_iterator rbegin() { return inst_list.rbegin(); };//2012679 add
    std::vector<MachineInstruction*>::reverse_iterator rend() { return inst_list.rend(); };//2012679 add
    ...
    void output();
    int getCmpNo() const { return cmpno; };//2012679 增 获取前一条 CmpInstruction 的条件码
    void setCmpNo(int cond) { cmpno = cond; };//2012679 增 设置前一条 CmpInstruction 的条件码
    MachineFunction* getParent() const { return parent; };//2012679 增 
};
...
public:
    ...
    * you can use this function in LinearScan::genSpillCode() */
    ...
    void output();
    std::vector<MachineOperand*> getSavedRegs();//2012679 增加函数
    int getParamsNum() const { return paramsNum; };//2012679 增
    MachineUnit* getParent() const { return parent; };//2012679 增 
};

class MachineUnit 
{
private:
    std::vector<SymbolEntry*> global_list;//增
